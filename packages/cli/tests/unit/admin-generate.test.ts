...
/**
 * Fixture: a manifest mounting @voyant-travel/foo (which has a conventional
 * @voyant-travel/foo-react/admin entry) and @voyant-travel/bar (no ui package at all).
 */
...
describe("adminGenerateCommand", () => {
  ...
  it("skips modules without an admin entry and notes them", async () => {
    ...
    const content = readFileSync(join(tmp, "src", "admin.extensions.generated.ts"), "utf8")
    expect(content).not.toContain("@voyant-travel/bar")
    expect(stderr.join("")).toContain("[admin-generate]")
    expect(stderr.join("")).toContain("@voyant-travel/bar")
  })

  it("warns and skips when the admin entry lacks the conventional export", async () => {
    writeFixture(tmp)
    writeFileSync(
      join(tmp, "node_modules", "@voyant-travel", "foo-react", "src", "admin", "index.tsx"),
    ...
    const content = readFileSync(join(tmp, "src", "admin.extensions.generated.ts"), "utf8")
    expect(content).not.toContain("@voyant-travel/foo-react/admin")
  ...
  it("honors a package.json voyant.adminEntry override", async () => {
    writeFileSync(
      ...
      `export default { modules: ["@voyant-travel/baz"] }\n`,
    )
    writePackage(tmp, "@voyant-travel/baz", {
      exports: { ".": "./src/index.ts" },
      voyant: { adminEntry: "@voyant-travel/baz-admin/entry" },
    })
    writePackage(
      tmp,
      "@voyant-travel/baz-admin",
    ...
    const content = readFileSync(join(tmp, "src", "admin.extensions.generated.ts"), "utf8")
    expect(content).toContain(
      `import { createBazAdminExtension } from "@voyant-travel/baz-admin/entry"`,
    )
...
describe("adminGenerateCommand --routes — redirects + nested children", () => {
  ...
  beforeEach(() => {
    ...
    writePackage(tmp, "@voyant-travel/foo", { exports: { ".": "./src/index.ts" } })
    writePackage(
      tmp,
      "@voyant-travel/foo-react",
      { exports: { ".": "./src/index.ts", "./admin": "./src/admin/index.tsx" } },
...
describe("adminGenerateCommand --routes — built-in core entry", () => {
  ...
  it("includes the core section when @voyant-travel/admin-app exposes ./core-extension", async () => {
